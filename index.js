...
async function replicateTable (tableName, primaryKeyField, targetDb, sqlPool, config) {
    console.log('replicateTable:' + tableName);
...
async function main () {
	...
	const msdbconfig  =	{
							...
							database: 'test_maxis_office_manager_0101_20200701_20201220'	,
							requestTimeout		: 90000	,
							connectionTimeou	: 90000	,
							options	: {encrypt: false}
						}
		;
	var ca = [fs.readFileSync(__dirname + "/ca-cert.crt")];
	var cert = fs.readFileSync(__dirname + "/m148.pem");
	var key = fs.readFileSync(__dirname + "/m148-private.key");
    ...
    const mongoClient = await mongodb.MongoClient.connect(config.mongoConnectionString, mongoCert);
    const targetDb = mongoClient.db(config.targetDatabaseName);
    

    const sqlPool = await sql.connect(msdbconfig);
